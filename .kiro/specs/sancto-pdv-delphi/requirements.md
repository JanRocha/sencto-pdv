...
Este documento especifica os requisitos para o sistema SANCTO PDV, desenvolvido em Delphi 10.2 Tokyo com interface FireMonkey (FMX). O sistema é um Ponto de Venda (PDV) unificado para parques temáticos infantis indoor localizados em shopping centers. A solução é composta por um **único executável** (`SanctoPDV_PDV.exe`) que centraliza tanto as operações diárias (vendas, caixa, visitantes, festas) quanto a administração (produtos, colaboradores, fiscal, relatórios, configurações), com controle de acesso baseado em papéis.

## Glossary

- **Sistema_PDV**: Aplicativo executável SanctoPDV_PDV.exe responsável por todas as operações e administração do parque
...
- **Módulo Operacional**: Conjunto de telas acessíveis a todos os papéis (Vendas, Caixa, Visitantes, Festas)
- **Módulo Administrativo**: Conjunto de telas acessíveis apenas a ADMINISTRADOR e GERENTE (Dashboard, Produtos, Colaboradores, Fiscal, Relatórios, Configurações)
...
1. WHEN um Colaborador informa CPF e senha válidos e a conta está ativa e não bloqueada, THE Sistema_PDV SHALL autenticar o Colaborador, registrar a data e hora do acesso e exibir a tela principal com as funções correspondentes ao papel do Colaborador.
...
5. WHEN um Colaborador com papel OPERACIONAL realiza login, THE Sistema_PDV SHALL exibir apenas as funções do Módulo Operacional: Vendas, Caixa e Visitantes.
6. WHEN um Colaborador com papel ADMINISTRADOR ou GERENTE realiza login, THE Sistema_PDV SHALL exibir todas as funções do Módulo Operacional e do Módulo Administrativo: Dashboard, Vendas, Caixa, Visitantes, Festas, Produtos, Colaboradores, Fiscal, Relatórios e Configurações.
7. WHEN um Colaborador realiza logout, THE Sistema_PDV SHALL encerrar a sessão, registrar o horário de saída no log de auditoria e retornar à tela de login.
8. IF um Colaborador com conta inativa tenta realizar login, THEN THE Sistema_PDV SHALL rejeitar a autenticação e exibir mensagem de erro genérica sem revelar o motivo específico da rejeição.
9. IF a sessão de um Colaborador permanece inativa por mais de 30 minutos, THEN THE Sistema_PDV SHALL encerrar a sessão automaticamente e redirecionar o Colaborador para a tela de login.
...
1. WHEN um ADMINISTRADOR solicita cadastro de novo Colaborador, THE Sistema_PDV SHALL exigir nome completo (mínimo 3, máximo 120 caracteres), CPF (11 dígitos numéricos válidos), email (formato válido, máximo 255 caracteres), telefone (mínimo 10, máximo 11 dígitos), senha (mínimo 6 caracteres) e papel (ADMINISTRADOR, GERENTE ou OPERACIONAL).
2. WHEN um ADMINISTRADOR submete o formulário de cadastro com CPF já existente, THE Sistema_PDV SHALL rejeitar o cadastro e exibir mensagem informando que o CPF já está registrado.
3. WHEN um ADMINISTRADOR submete o formulário de cadastro com email já existente, THE Sistema_PDV SHALL rejeitar o cadastro e exibir mensagem informando que o email já está registrado.
4. WHEN um ADMINISTRADOR solicita desativação de um Colaborador diferente de si próprio, THE Sistema_PDV SHALL definir o status do Colaborador como inativo e impedir login futuro sem excluir o registro.
5. WHEN um ADMINISTRADOR edita os dados de um Colaborador, THE Sistema_PDV SHALL permitir alteração de nome completo, email, telefone, papel e senha, manter o CPF inalterável, salvar as alterações e registrar a modificação no log de auditoria.
6. THE Sistema_PDV SHALL armazenar senhas de Colaboradores utilizando hash criptográfico unidirecional.
7. WHEN um ADMINISTRADOR ou GERENTE solicita reativação de um Colaborador bloqueado por 5 tentativas de login falhas consecutivas, THE Sistema_PDV SHALL zerar o contador de tentativas, remover a data de bloqueio e desbloquear a conta imediatamente.
8. IF um ADMINISTRADOR tenta desativar a própria conta, THEN THE Sistema_PDV SHALL rejeitar a operação e exibir mensagem indicando que não é permitido desativar o próprio usuário.
...
1. WHEN um ADMINISTRADOR ou GERENTE cadastra um novo Produto, THE Sistema_PDV SHALL exigir nome (máximo 120 caracteres), código de barras (formato EAN-8, EAN-13 ou código interno de até 20 caracteres), categoria, preço de venda (entre 0,01 e 999.999,99), unidade, tipo (PRODUTO, SERVIÇO ou INGRESSO), estoque inicial (0 a 99.999), estoque mínimo (0 a 99.999) e campos fiscais (NCM com 8 dígitos, CFOP com 4 dígitos, CST/CSOSN, alíquotas ICMS/PIS/COFINS entre 0,00% e 100,00%).
2. WHEN um ADMINISTRADOR ou GERENTE submete o formulário com código de barras já existente, THE Sistema_PDV SHALL rejeitar o cadastro e exibir mensagem informando duplicidade do código de barras.
3. WHEN um ADMINISTRADOR ou GERENTE solicita exclusão de um Produto que possui itens de venda ou itens de visita vinculados, THE Sistema_PDV SHALL negar a exclusão e oferecer a opção de desativação.
4. WHEN um ADMINISTRADOR ou GERENTE desativa um Produto, THE Sistema_PDV SHALL definir o status como inativo e ocultar o Produto da tela de vendas.
...
6. WHEN um ADMINISTRADOR ou GERENTE importa planilha Excel (.xlsx) de produtos com até 5.000 linhas, THE Sistema_PDV SHALL validar cada linha conforme as regras do critério 1, inserir produtos válidos, e gerar relatório de erros listando número da linha, campo inválido e motivo da rejeição para cada linha inválida.
7. IF a planilha importada contém mais de 5.000 linhas, THEN THE Sistema_PDV SHALL rejeitar a importação e exibir mensagem informando o limite máximo de linhas permitido.
8. WHEN um ADMINISTRADOR ou GERENTE solicita exportação de produtos, THE Sistema_PDV SHALL gerar arquivo Excel (.xlsx) contendo todos os produtos ativos com seus respectivos campos fiscais.
9. THE Sistema_PDV SHALL organizar Produtos em Categorias e permitir filtro por Categoria na interface de vendas.
10. IF um ADMINISTRADOR ou GERENTE submete o formulário de cadastro com campos fiscais em formato inválido (NCM diferente de 8 dígitos numéricos ou CFOP diferente de 4 dígitos numéricos), THEN THE Sistema_PDV SHALL rejeitar o cadastro e indicar os campos com formato incorreto.
...
6. WHEN a diferença absoluta entre saldo esperado e contagem física exceder o valor de tolerância configurado (padrão R$ 5,00), THE Sistema_PDV SHALL destacar a diferença em cor vermelha e exigir justificativa do Colaborador (mínimo 10 caracteres).
...
1. WHEN um Colaborador seleciona um Produto ativo na tela de vendas, THE Sistema_PDV SHALL adicionar o Produto ao carrinho com quantidade 1 e preço de venda vigente (preço promocional se disponível, caso contrário preço de venda).
...
8. WHEN um ADMINISTRADOR cadastra ou edita um Pacote_Festa, THE Sistema_PDV SHALL exigir nome (máximo 80 caracteres), número máximo de convidados (1 a 200), preço dia de semana (R$ 0,01 a R$ 99.999,99), preço fim de semana (R$ 0,01 a R$ 99.999,99) e descrição (máximo 500 caracteres).
...
6. WHEN um ADMINISTRADOR configura certificado digital, THE Sistema_PDV SHALL validar o tipo do certificado (A1 ou A3), verificar que a data de validade não está expirada, confirmar leitura da chave privada e armazenar as informações de forma segura.
7. WHEN o certificado digital está a 30 dias ou menos da expiração, THE Sistema_PDV SHALL exibir alerta visual ao ADMINISTRADOR a cada login informando a data de vencimento do certificado.
8. THE Sistema_PDV SHALL permitir configuração do ambiente SEFAZ (homologação ou produção), série (1 a 999) e número inicial de numeração de notas (1 a 999.999.999).
...
1. WHEN um ADMINISTRADOR ou GERENTE solicita relatório de vendas, THE Sistema_PDV SHALL gerar relatório com filtros por período (data início e data fim, máximo 365 dias), Colaborador, forma de pagamento e Categoria de produto, exibindo total de vendas, quantidade de transações e valor médio por transação.
2. WHEN um ADMINISTRADOR ou GERENTE solicita relatório de caixa, THE Sistema_PDV SHALL exibir resumo de aberturas, fechamentos, sangrias, suprimentos e diferenças (valor esperado vs. contagem física) filtráveis por período e por Colaborador.
3. WHEN um ADMINISTRADOR ou GERENTE solicita relatório de visitantes, THE Sistema_PDV SHALL exibir quantidade total de visitas, tempo médio de permanência (em minutos), receita total de consumo e receita média por visitante, filtráveis por período.
4. WHEN um ADMINISTRADOR ou GERENTE acessa o dashboard, THE Sistema_PDV SHALL exibir indicadores do dia corrente atualizados a cada acesso: total de vendas em reais, número de visitantes com status ABERTA, quantidade de caixas abertos e quantidade de alertas pendentes (estoque baixo, certificado expirando).
5. WHEN um ADMINISTRADOR ou GERENTE solicita relatório de produtos, THE Sistema_PDV SHALL exibir ranking dos 50 produtos mais vendidos por quantidade no período, lista de produtos com estoque atual igual ou abaixo do estoque mínimo e lista de produtos com status inativo.
6. WHEN um ADMINISTRADOR ou GERENTE solicita exportação de relatório, THE Sistema_PDV SHALL gerar arquivo no formato selecionado (PDF ou Excel/XLSX) contendo os mesmos dados e filtros aplicados na visualização em tela, em até 30 segundos para relatórios de até 10.000 registros.
...
4. THE Sistema_PDV SHALL permitir configuração do número de minutos antes do vencimento para disparo de alerta (valor entre 1 e 60 minutos, padrão: 10 minutos).
5. THE Sistema_PDV SHALL permitir configuração das credenciais da WhatsApp_API (token de acesso, phone number ID e template ID) com validação de campos obrigatórios não vazios.
...
1. WHEN um ADMINISTRADOR solicita sincronização de produtos com ERP_Omie, THE Sistema_PDV SHALL enviar produtos novos ou alterados desde a última sincronização via RESTRequest4Delphi, registrar para cada produto o resultado (sucesso ou falha com mensagem de erro) e exibir resumo ao final com total enviado, total com sucesso e total com falha.
2. WHEN um ADMINISTRADOR solicita sincronização de clientes com ERP_Omie, THE Sistema_PDV SHALL enviar dados de Tutores cadastrados ou alterados desde a última sincronização e registrar o resultado individual (sucesso ou falha) para cada Tutor.
...
4. IF a comunicação com ERP_Omie falha durante sincronização (timeout de 30 segundos ou erro de rede), THEN THE Sistema_PDV SHALL registrar o erro com data/hora e mensagem, manter os dados locais íntegros sem alteração, marcar os itens pendentes para retentativa e permitir retentativa manual pelo ADMINISTRADOR.
5. THE Sistema_PDV SHALL permitir configuração das credenciais do ERP_Omie (app_key, app_secret, endpoint URL) com validação de campos obrigatórios não vazios.
...
4. THE Sistema_PDV SHALL seguir o padrão arquitetural Model → Controller → DAO (FenixORM) → View (FMX Forms), sem acesso direto ao banco de dados a partir das Views.
...
8. THE Sistema_PDV SHALL implementar todas as telas como par de arquivos .fmx (layout visual) + .pas (código), com todos os componentes visuais posicionados em design-time no Form Designer da IDE, sem criação de componentes em tempo de execução. A visibilidade de elementos pode ser alterada em runtime para controle de acesso por papel, mas o componente deve existir no .fmx.
...
2. WHEN um ADMINISTRADOR ou GERENTE consulta o log de auditoria, THE Sistema_PDV SHALL exibir registros com filtros por Colaborador, tipo de ação, entidade e período (data início e data fim), ordenados por data/hora decrescente, com paginação de 50 registros por página.
...
4. THE Sistema_PDV SHALL impedir exclusão ou alteração de registros do log de auditoria por qualquer papel de Colaborador, incluindo ADMINISTRADOR.
...
1. THE Sistema_PDV SHALL permitir configuração do banco de dados: tipo (Firebird ou SQLite), caminho do arquivo (máximo 260 caracteres) e credenciais quando o tipo for Firebird.
2. THE Sistema_PDV SHALL permitir configuração do modo de operação do FenixORM (local, cloud ou híbrido) com URL base do servidor (máximo 255 caracteres) e credenciais de autenticação para modo cloud.
3. THE Sistema_PDV SHALL permitir configuração dos parâmetros fiscais: ambiente SEFAZ (homologação ou produção), série (1 a 999), certificado digital (arquivo A1 ou dispositivo A3) e CNPJ emitente (14 dígitos com validação de dígito verificador).
4. THE Sistema_PDV SHALL permitir configuração dos parâmetros de alerta de visitantes: minutos antes do vencimento para alerta visual (1 a 60, padrão: 5) e minutos antes do vencimento para notificação WhatsApp (1 a 60, padrão: 10).
5. THE Sistema_PDV SHALL permitir configuração do valor de tolerância para diferença de caixa no fechamento (R$ 0,00 a R$ 100,00, padrão: R$ 5,00).
6. WHEN um ADMINISTRADOR altera qualquer configuração do sistema, THE Sistema_PDV SHALL validar os novos valores conforme regras de cada parâmetro, registrar a alteração no log de auditoria (valor anterior e novo valor) e aplicar a configuração imediatamente sem necessidade de reiniciar o aplicativo.
